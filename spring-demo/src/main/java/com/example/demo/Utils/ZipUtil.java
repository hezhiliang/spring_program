...
public class ZipUtil {
    ...
    private static List<File> getAllFiles(File srcFile) {
        ...
        File[]     tmp      = srcFile.listFiles();//将srcFile目录下的所有文件或文件夹的路径添加入tmp数组
        System.out.println("[" + srcFile.getName() +  "]文件夹下有 :" + tmp.length + " 个文件.");
        for (int i = 0; i < tmp.length; i++) {
            ...
            if (tmp[i].isDirectory()) {
                ...
                if (tmp[i].listFiles().length!=0){
                    System.out.println("执行递归的路径为:" + tmp[i] );
                    fileList.addAll(getAllFiles(tmp[i]));//addAll方法用于将指定 collection 中的所有元素添加到列表,如果List集合对象由于调用addAll方法而发生更改，则返回true。
                }
    ...
    /**
     * 解压缩方法
     * @param zipFileName 被压缩文件的路径
     * @param dstPath 解压后文件的路径
     *
     * @return
     */
     ...
     * （可以压缩空的子目录）
     * @param srcPath 压缩源路径
     * @param zipFileName 目标文件压缩后的压缩包文件名
     *
     * @return
     */
...
//        List<File> allFiles = getAllFiles(new File("C:\\Users\\x1c\\Desktop\\nbzyb图表"));
//        for (int i = 0; i < allFiles.size(); i++) {
//            System.out.println("list集合中第" + i + "个元素是:" + allFiles.get(i));
//        }
//        System.out.println("-----------------");

//        String relativePath = getRelativePath("C:\\Users\\x1c\\Desktop\\nbzyb图表", new File("C:\\Users\\x1c\\Desktop\\nbzyb图表\\一层\\321.docx"));
//        System.out.println(relativePath);
        boolean unzip = unzip("E:\\spring-demo\\压缩测试文件2.zip", "E:\\spring-demo\\压缩测试文件2");
